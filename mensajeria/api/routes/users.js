...
router.get("/friends/:userId", async (req, res) => {
  try {
    ...
    let friendList = [];
    friends.map((friend) => {
      const { _id, username, profilePicture, nombre, apellidoP, apellidoM } = friend;
      friendList.push({ _id, username, profilePicture, nombre, apellidoP, apellidoM });
    });
  ...
  if (req.body.userId !== req.params.id) {
    try {
      ...
      if (user.followers.includes(req.body.userId)) {
        ...
        res.status(200).json("el usuario ha sido dejado de seguir");
      } else {
